...
from .tools import create_google_form, setup_form_settings, add_questions_to_form
...
form_creator_agent = Agent(
    ...
    **WORKFLOW MANDATE**: You MUST follow this sequence of operations without deviation:
    1.  **CREATE THE FORM**: First, you MUST call the `create_google_form` tool. This is the only way to get a `form_id`. You MUST provide both a `title` and a `description`.
    ...
    **AUTO-COMPLETION BEHAVIOR**:
    ...
    - If no description is provided, create an appropriate one automatically.
    - If form settings aren't specified, use sensible defaults (e.g., collect emails).
    
    **IMPORTANT**: The `create_google_form` function requires both title and description parameters.
    ...
    tools=[create_google_form, setup_form_settings, add_questions_to_form],
